...
while read _pkg_ _ver_; do
	if command -v $_pkg_ &>/dev/null; then
		if [[ $($_pkg_ --version 2>&1) =~ [0-9]+\.[0-9]+ ]]; then
			...
			printf "%s: error: '%s' Unable to get version.\n" ${_SHELLBOT_[name]} $_pkg_ 1>&2
			exit 1
		fi
	else
...
readonly _ERR_SERVICE_EXISTS_='error when creating the service: the service name already exists.'
...
MessageError()
{
	...
	# Finaliza script/thread em caso de error interno, caso contrário retorna 1
...
CheckArgType()
{
	...
	# de error é retornada e o script/thread é finalizado com status '1'.
	case $1 in
