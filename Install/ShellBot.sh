...
#						obrigatórios retornam uma mensagem de error caso o argumento seja omitido.
#					
...
if ! awk 'BEGIN { exit ARGV[1] < 4.3 }' ${BASH_VERSINFO[0]}.${BASH_VERSINFO[1]}; then
	echo "${BASH_SOURCE:-${0##*/}}: error: requires command interpreter 'bash 4.3' ou superior." 1>&2
	exit 1
fi
...
[description]='API unofficial for creating bots on the Telegram platform.'
[version]='6.4.0'
	...
	if command -v $_pkg_ &>/dev/null; then
		if [[ $($_pkg_ --version 2>&1) =~ [0-9]+\.[0-9]+ ]]; then
			if ! awk 'BEGIN { exit ARGV[1] < ARGV[2] }' $BASH_REMATCH $_ver_; then
				printf "%s: errorr: requires package '%s %s' or superior.\n" ${_SHELLBOT_[name]} $_pkg_ $_ver_ 1>&2
				exit 1
			fi
		else
			printf "%s: errorr: '%s' Unable to get version.\n" ${_SHELLBOT_[name]} $_pkg_ 1>&2
			exit 1
		fi
	else
		printf "%s: errorr: '%s' the required package is missing.\n" ${_SHELLBOT_[name]} $_pkg_ 1>&2
		exit 1
	fi
...
readonly _ERR_TYPE_FLOAT_='incompatible type: supports float only.'
...
readonly _ERR_FILE_NOT_FOUND_='Access failed: Unable to read the file.'
...
readonly _ERR_UNKNOWN_='unknown errorr: An unexpected failure occurred. Report the problem to the developer.'
readonly _ERR_SERVICE_NOT_ROOT_='access denied: requires root privileges.'
readonly _ERR_SERVICE_EXISTS_='errorr creating service: service name already exists.'
readonly _ERR_SERVICE_SYSTEMD_NOT_FOUND_='errorr when activating: the system does not support "systemd" service management.'
...
readonly _ERR_HANDLE_EXISTS_='errorr when registering: there is already a handle linked to the callback'
...
MessageError()
{
	...
	# TG - External errorr returned by the Telegram core.
	# API - Internal errorr generated by the ShellBot API.
	case $1 in
		TG)
			err_param="$(Json '.errorr_code' "$2")"
	...
	# Terminates script/thread in case of internal errorr, otherwise returns 1
...
CheckArgType()
{
	...
	# errorr is returned and the script/thread ends with status '1'.
	case $1 in
...
CreateUnitService()
{
	...
	# 'service' mode requires the 'systemd' process management system
...
ShellBot.init()
{
    ...
    ShellBot.ReplyKeyboardMarkup()
    {
    	...
    	# Imprime mensagem de error se o parâmetro obrigatório for omitido.
    	[[ $__button ]] || MessageError API "$_ERR_PARAM_REQUIRED_" "[-b, --button]"
...
_EOF
    ...
    ShellBot.getUserProfilePhotos()
    {
    	...
    	# Verifica se ocorreu errors durante a chamada do método	
    	MethodReturn "$jq_obj" || MessageError TG "$jq_obj"
	...
	ShellBot.downloadFile()
	{
		...
		# destino é retornado, caso contrário uma mensagem de error é apresentada.
		if wget -qO "$dir/$file" "$uri/$file_path"; then
			# Sucesso
			printf -v jq_obj '{"ok":true,"result":{"file_path":"%s"}}' "$dir/$file"
		else
			# Falha
			printf -v jq_obj '{"ok":false,"errorr_code":404,"description":"Bad Request: file not found"}'
